...
- Requirement > usable capacity: OFFLOAD if a separate configured pool could accommodate the deficit, otherwise DOES_NOT_FIT. Both fail the default must-fit constraint. This is not validation of offload support or speed.
...
Local load power = 90% normal + 10% peak; all figures are whole-system assumptions. Electricity = (load power × load hours + idle watts × idle hours) / 1000 × USD/kWh. Idle hours = enabled window minus load; enabling always-on changes that window to 730 h. Off-hours are otherwise powered off. Capex persists regardless of usage.

Cloud/managed business-hours and continuous schedules bill the full configured active window, even if utilization is zero. Bursty/batch profiles assume scheduled shutdown outside load hours. Always-warm overrides this with 730 h. This is an idealized policy, not a provider feature guarantee. Managed endpoint base hourly price × (1 + explicit 30% premium); VM has zero premium. Storage and egress are configured fixed monthly amounts and default to zero. Token APIs have no idle token fee in this profile.
